...
class RBCUtils:
    ...
    def runCmd (self, cmds):
        ...
        result = subprocess.run(cmds, shell=True, cwd=VBoxPath, capture_output=True, text=True)
    ...
    def StopRBCOnHost(self):
        ...
        cmds = f'sudo rm -rf /MooNSimu/LDS/Executables/*'
        self.sendCmd(cmds)
    ...
    def StopVM(self):
        ...
        self.runCmd(['VBoxManage', 'controlvm', self._RBCName, 'poweroff'])
    ...
    def StartVM(self):
        ...
        self.runCmd(['VBoxManage', 'modifyvm', self._RBCName, '--nic2', 'hostonly',
                                '--hostonlyadapter2', InterfaceName1])

        self.runCmd(['VBoxManage', 'modifyvm', self._RBCName, '--nic3', 'intnet', '--intnet2',
                                InterfaceName2])

        self.runCmd(['VBoxManage', 'modifyvm', self._RBCName, '--nic4', 'hostonly',
                                '--hostonlyadapter4', InterfaceName3])

        self.runCmd(['VBoxManage', 'startvm', self._RBCName, '--type',
                                 'headless'])
        ...
        while retry < num_retries:
            try:
                self._client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                self._client.connect(self._RBCip, port=self._RBCport,  username=self._RBCuser, password=self._RBCpass,  timeout=2)
                scp = SCPClient(self._client.get_transport())
                retry = num_retries
            except socket.error:
                self.log(str(socket.error))
                ...
                if retry == num_retries:
                    self.log('Leaving the script, please retry to launch it. ' + self._RBCName )
                    sys.exit()
        ...
        cmds = f'sudo mkdir /MooNSimu'  #'/MooNSimu': File exists
        self.sendCmd(cmds)
        ...
        cmds = f'sudo mkdir /MooNSimu/LDS' #'/MooNSimu/LDS': File exists
        self.sendCmd(cmds)
        cmds = f'sudo mkdir /home/admin/LDS_BSL5.5/scripts' #'/home/admin/LDS_BSL5.5/scripts': File exists
        self.sendCmd(cmds)
        cmds = f'sudo mkdir /etc/default/alchemist_lib' #'/etc/default/alchemist_lib': File exists
        self.sendCmd(cmds)
        cmds = f'sudo mkdir /etc/default/logger_lib'
        self.sendCmd(cmds)

        self.log('LDS Simulator ' + self._RBCName + ' set permission directory start')
        cmds = f'sudo chmod -R 777 /home/admin/LDS_BSL5.5'
        self.sendCmd(cmds)
        ...
        self.sendCmd(cmds)
        self.log('LDS Simulator ' + self._RBCName + ' set permission directory finish')
...
if __name__ == "__main__":
    ...
    if action == "help":
        ...
        print('python.exe c:/gitrepo/RadioBlockCenterTestEnv/RBCUtils.py start "C:\\ETCS2-TRK_TestEnv_93B\\Simu\\MooN_Simu\\RBC5\\" RBC_MooN_93000 RBC5_93B 192.168.56.55')
        print('python.exe c:/gitrepo/RadioBlockCenterTestEnv/RBCUtils.py start "C:\\ETCS2-TRK_TestEnv_93B\\Simu\\MooN_Simu\\RBC5\\" RBC_MooN_93000 IMP_Gateway_93A 192.168.56.3 imp_gateway impgateway')
